...
-- DATABASE 접속 SHOPPING 데이터 베이스 접속 후 PRODUCT 테이블 생성 하고 생성 확인하기
USE KHTSHOPPING;
...
-- ORDER 는 SQL 예약어인 ORDER BY 이름과 충돌나기 때문에 백틱(``)으로 감싸야함
CREATE TABLE `ORDER` (
...
INSERT INTO REVIEW (orderorderorderPRODUCT_ID, USER_ID, RATING, COMMENT) VALUES
...
-- KHTUSER 데이터 베이스 --
USE KHTUSER;
-- USER TABLE 홍길동의 이메일 주소와 전화번호 조회
SELECT EMAIL, PHONE
FROM USER
WHERE USENAME = '홍길동';
...
-- KHTSHOPPING 데이터 베이스 --
USE KHTSHOPPINT;
-- `ORDER` 테이블 주문상태가 PENDING 인 주문 수를 조회 오직 주문수 alter
SELECT COUNT(*)
FROM `ORDER`
...
FROM CAFE_ORDER
WHERE STAYUS = 'PREPARING';


-- KHTUSER 데이터 베이스에서 LOGIN 활동을 한 사용자의 이름과 세부활동 정보 확인alter
-- JOIN ON
USE KHTUSER;
...
-- UPDATE 문을 SQL 에서 집적적으로 실행할 때 문제가 되는 것을 방지하기위해 SAFE 모드 생성
-- 테이블을 삭제할 때도 마찬가지로 SAFE 모드 이므로 SALE MODE 해지 후 실행 가능
...
UPDATE USER SET PASSWORD_HASH = 'CHULSOO' WHERE USERNAME = '이철수';
...
-- CREATE TABLE ENUM = RADIOS

-- 테이블자체 컬럼에 존재하는 규칙 수정
-- ALTER TABLE 수정할 테이블 명칭 MODIFY COLUMN 컬럼명 컬럼명의 규칙
ALTER TABLE TABLES
MODIFY COLUMN STATUS ENUM ('AVAILABLE', 'OPPCUPIED', '사용가능','사용중');
...
ALTER TABLE TABLES
MODIFY COLUMN STATUS ENUM ('사용가능','사용중');
...
UPDATE CAFE_ORDER
SET STATUS = '준비중', TOTAL_AMOUNT = 8500
WHERE TABLE_ID = 2;

UPDATE CAFE_ORDER
SET STATUS = '완료',TOTAL_AMOUT=13500
WHERE TABLE_ID =4;

UPDATE CAFE_ORDER
SET STATUS = '대기중',TOTAL_AMOUT=5000
WHERE TABLE_ID =1;
...
UPDATE CAFE_ORDER
SET STATUS = '취소',TOTAL_AMOUT=4000
WHERE TABLE_ID =5;
