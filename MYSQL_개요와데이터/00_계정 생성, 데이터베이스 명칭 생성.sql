/*
cmd(명령 프롬포트 창) 에서 mysql 데이터베이스명칭, 유저생성, 권한생성 접속
mysql -u root -p 
...
'%'         = 자바에서 public 와 같은 개넘으로, 어떤 컴퓨터에서든 접근 가능
'localhost' = 자바에서 private 와 같은 개념으로, 현재 사용하는 컴퓨터에서만 접근 가능 

권한 허용 / 모든 특권을 / on 데이터베이스* 내에 있는 모든 테이블 접근 ok / 어떤 계정에 부여하는가.
GRANT / ALL PRIVILEGES / ON 데이터베이스명칭.*                        /TO 'kht'@'%';
