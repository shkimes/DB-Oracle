...
고정 길이
CHAR(n) : 길이가 고정적으로 설정
        무조건 길이가 짧든 적든 초기에 작성한 길이만큼 공건 설정
        
가변 길이
VARCHAR : 변수 공간의 길이를 변경가능한 문자열
            실제 데이터의 길이만큼 저장
            최대 255자(숫자, 영어 기준) 저장 가능
TEXT        :  긴 글자를 저장할 때 사용                 최대 65,646 자 까지 저장 가능 
MEDIUMTEXT  : TEXT 보다 큰 텍스트를 저장                최대 16MB
LONGTEXT    : TEXT, MEDIUMTEXT 보다 큰 텍스트를 저장    최대 5GB

기타 자료형

BLOB : 이미지, 동영상 등과 같은 이진 데이터 저장
        미리 정의된 값을 선택하는데 사용 (택 1가지 선택)
        HTML에서 SELECT - OPTION
        RADIO 처럼 지정된 범위에서만 선택 가능
        EX. ENUM('small',medium','large')=작음 중간 큼 중 하나를 선택해서 DB
...
TIMESTAMP : 날짜와 시간을 함께 저장하는데, 값이 연동될 때 자동으로 현재시간으로 갱신
